...
public class UserDetailsServiceImpl implements UserDetailsService {
    ...
    public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
        OrdinaryUser user = null; OrdinaryUser user0 = null;
        switch (username) {
            case "admin":
                user = new OrdinaryUser(1, "admin", new BCryptPasswordEncoder().encode("123456"), "1");

             //   user.setRoles(ADMIN_ROLES);
                break;
            default:
                user = new OrdinaryUser(2,"grandhappy",new BCryptPasswordEncoder().encode("123456"),"1");
            //    user.setRoles(COMMON_ROLES);
        }
        System.out.println(username+"用户id:"+user.getUserId()+"用户名字："+user.getRealName()+"用户密码："+user.getPassword());

       user0 = userService.findByUsername(username);
        System.out.println(username+"用户id:"+user0.getUserId()+"用户名字："+user0.getRealName()+"用户密码："+user0.getPassword());
       if(user0!=null) {
           user = new OrdinaryUser(user0.getUserId(), user0.getLoginName(), user0.getPassword(), user0.getStatus());
       }
        System.out.println(username+"用户id:"+user.getUserId()+"用户名字："+user.getRealName()+"用户密码："+user.getPassword());
